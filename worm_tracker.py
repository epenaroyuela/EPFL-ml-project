...
def parse_args(args):
    ...
    parser.add_argument('-I', '--input',
                        help='path of the input capture',
                        required=True)
    parser.add_argument('-O', '--output',
                        help='path of the output labels',
                        required=True)
    parser.add_argument('-A', '--annotation',
                        help='path of the annotated output capture',
                        required=False)
    parser.add_argument('-C', '--configuration',
                        help='path of the custom configuration',
                        required=False)
...
def preprocess_capture(capture, configuration):
    ...
    if configuration['remove_outside_petri']['apply']:
        capture.apply(remove_outside_petri(configuration['remove_outside_petri']['center'], configuration['remove_outside_petri']['radius']))
    return capture
...
def background_subtraction(capture, configuration):
    ...
    background_subtractor = cv2.createBackgroundSubtractorMOG2(history=configuration['history'], varThreshold=configuration['varTreshold'], detectShadows=False)
...
def trackpy_locate(capture, configuration):
    ...
    for i, frame in capture.frames():
        tmp_locations = tp.locate(frame[:, :, 0], configuration['diameter'], **locate_options)
...
def trackpy_link(locations, configuration):
    tp.quiet()
    links = tp.link(locations, configuration['search_range'], memory=configuration['memory'])
    return links
...
def find_worm(links, configuration):
    ...
    worm = worm.sort_values(['frame', 'particle_y'], ascending=[True, False]).drop_duplicates('frame')
    worm = worm.drop(columns=['particle_x', 'particle_y'])
    worm['particle'] = 0
    return worm

def compute_labels(worm, capture):
    worm = worm.set_index('frame').reindex(np.arange(capture.length())).reset_index().ffill()
    labels = {int(row['frame']): np.array([row['x'], row['y']]) for _, row in worm.iterrows()}
    return labels
...
def main():
    ...
    assert capture.W() == configuration['capture']['width'] and capture.H() == configuration['capture']['height']
    ...
    if arg_annotation is not None:
        ...
        annotation.apply(annotate(map_labels(labels, lambda l: larr2i(l)), color=[0, 255, 0]))
