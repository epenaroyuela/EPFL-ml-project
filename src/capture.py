"""Classes to manipulate captures"""
import copy
...
class LazyCapture:
    """Lazy abstraction of a set of ordered frames, enables to manipulate it at a high level"""
    ...
    @classmethod
    def load(cls, path):
        """Loads frames from avi file or folder of jpeg, returns a capture"""
    ...
    def random_load(cls, path):
        """Loads frames from avi file or folder of jpeg, in a random order, returns a capture"""
        length, W, H, C = None, None, None, None
        cap = cv2.VideoCapture(path)
        ret, _ = cap.read()
        if not ret:
    ...
    def concat(cls, captures):
        """Concatenates a set of captures, returns a capture"""
        ...
        for capture in captures[1:]:
            assert (
                capture._W == W and capture._H == H and capture._C == C
            ), "({}, {}, {}) != ({}, {}, {})".format(
                capture._W, capture._H, capture._C, W, H, C
            )
    ...
    def write(self, path, fps=50, limit=None, reverse=False):
        ...
        if self._length:
            ...
            for _, frame in (
                it.islice(self._frames(reverse=reverse), limit)
                if limit is not None
                else self._frames(reverse=reverse)
            ):
                out.write(frame)
            out.release()

    def __str__(self):
        return "LazyCapture[length = {}; shape = {}]".format(
            self._length, (self._W, self._H, self._C)
        )

    # Getters
    def length(self):
        """Returns length of the capture"""
        return self._length
    ...
    def C(self):
        """Returns the number of channels of the frames"""
        return self._C

    def frame(self, frame_no, index=True):
        """Returns the 'frame_no'-th of the capture, or the frame whose index is 'frame_no'"""
        if index:
            ...
            assert (
                frame_no >= 0 and frame_no < self._length
            ), "{} out of range [{}, {}]".format(frame_no, 0, self._length)
            it = self._frames(reverse=False)
            _skip(it, frame_no)
            return next(it)

    def frames_dict(self):
        """Returns a dictionary with the frames"""
    ...
    # Index
    def index(self):
        """Returns an iterator of the indices of the frames"""
    ...
    def frames(self, limit=None, reverse=False):
        """Returns an iterator of the frames"""
        return (
            it.islice(self._frames(reverse=reverse), limit)
            if limit is not None
            else self._frames(reverse=reverse)
    ...
    def filter(self, func):
        """Filters the frames according to the given function"""
    ...
    def extract(self, func):
        """Returns an iterator with the results of the function passed as parameter applied to every frame"""
        return iter((func(i, frame) for i, frame in self._frames(reverse=False)))

    def foreach(self, func, zip=None, acc=None, reverse=False):
        """Executes the function passed as parameter over every frame"""
        z, a = zip is not None, acc is not None
        assert not z or len(zip) == self._length, "len(zip) {} != {}".format(
            len(zip), self._length
        )
    ...
    def apply(self, func, zip=None, acc=None, reverse=False, shape=None):
        """Applies the function passed as parameter to every frame"""
    ...
    def rolling(self, func, window, zip=None, acc=None, reverse=False, shape=None):
        """Applies the function passed as parameter to a rolling window over the frames"""
        rolling_reverse = reverse
        assert window % 2 == 1, "window is not even"
        assert self._length >= window, "windos is too large, {} > {}".format(
            window, self._length
        )
        z, a = zip is not None, acc is not None
        assert not z or len(zip) == self._length, "len(zip) {} != {}".format(
...
class Capture:  # DEPRECATED
    """Abstraction of a set of ordered frames, enables to manipulate it at a high level. DEPRECATED."""
    ...
    def load(
        cls, path
    ):  # 'path' can be either a '.avi' or a wildcard for '.jpeg' images
        frames = []
        length, W, H, C = None, None, None, None
        cap = cv2.VideoCapture(path)
        ret, frame = cap.read()
    ...
    def clone(self):
        return Capture(
            self._length, self._W, self._H, self._C, copy.deepcopy(self._frames)
        )
    ...
    def __str__(self):
        return "Capture[length = {}; shape = {}]".format(
            self._length, (self._W, self._H, self._C)
        )
    ...
    def frames(self, limit=None, reverse=False):
        if reverse:
            return iter(
                reversed(self._frames[:limit] if limit is not None else self._frames)
            )
        else:
    ...
    def rolling(
        self, func, window, zip=None, acc=None, reverse=False, shape=None
    ):  # allow selecting position within window, allow lossless (fill ends with copies of end item), even window
        assert window % 2 == 1, "window is not even"
        assert self._length >= window, "windos is too large, {} > {}".format(
            window, self._length
        )
        ...
        half_window = window // 2
        q, lq = (
            (reversed(self._frames[-window + 1 :]), self._length - window)
            if reverse
            else (self._frames[: window - 1], window - 1)
        )
