...
class vibe_gray:
    def __init__(
        ...
        self.width = 0
        self.height = 0
        self.numberOfSamples = 20  # 100?
        self.matchingThreshold = 5
    ...
    def AllocInit(self, image):
        ...
        self.historyImage = np.zeros(
            (self.height, self.width, self.numberOfHistoryImages), np.uint8
        )
        for i in range(self.numberOfHistoryImages):
            self.historyImage[:, :, i] = image

        # create and fill the historyBuffer
        self.historyBuffer = np.zeros(
            (
                self.height,
                self.width,
                self.numberOfSamples - self.numberOfHistoryImages,
            ),
            np.uint8,
        )
        ...
        for i in range(size):
            ...
            self.neighbor[i] = (
                np.random.randint(-1, 3 - 1) + np.random.randint(-1, 3 - 1) * self.width
            )
    ...
    def Segmentation(self, image):
        ...
        # for swapping
        self.lastHistoryImageSwapped = (
            self.lastHistoryImageSwapped + 1
        ) % self.numberOfHistoryImages
        ...
        for i in range(numberOfTests):
            mask_ = (
                np.abs(image - self.historyBuffer[:, :, i]) <= self.matchingThreshold
            )
            mask_ = mask * mask_
        ...
        for y in range(1, self.height - 1):
            ...
            while indX < self.width - 1:
                ...
                if updating_mask[y, indX] == 255:
                    ...
                    y_, indX_ = int(index_neighbor / self.width), int(
                        index_neighbor % self.width
                    )
